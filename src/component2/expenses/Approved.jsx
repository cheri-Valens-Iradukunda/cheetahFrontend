...
export const ExpensesApproved = () => {
    ...
    const getAllPending = () => {
        setLoading(true)
        axios.get(Api.expenses.value +"/byStatus/"+date +"/"+number + "/notPending",{headers: Api.Token}).then(res => {
                ...
                deletedPending && deletedPending.map((res,index) => (
                        ...
                        <div className="col-lg-3 col-md-4 col-sm-6 col-12 mb-2 " key={index}>
                            <div className="p-2 border border-dark border-1 rounded" style={res.status=="rejected"?
                                {background:"rgb(253, 219, 219)"}:res.status=="pending"?{background:"rgb(241, 215, 143)"}:{background:"rgb(216, 233, 248)"}}>
